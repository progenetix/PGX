...
# local packages
# TODO: extract BeaconPlus dependencies & move to PGX
...
my $accessid = $config->{param}->{accessid}->[0];
...
my $api = {
	...
	callback =>	$config->{param}->{callback}->[0],
	plotargs =>	{ map{ $_ => join(',', @{ $config->{param}->{$_} }) } (grep{ /^\-\w+?$/ } keys %{ $config->{param} }) },
...
if ($config->{param}->{"do"}->[0] eq "cnvhistogram") {
